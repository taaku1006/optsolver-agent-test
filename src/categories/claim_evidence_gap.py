"""Claim-evidence gap category checker.

Detects claims that lack sufficient supporting evidence or where evidence
contradicts or doesn't fully support the claims made in the paper.
"""
...
class ClaimEvidenceGapChecker(BaseChecker):
    """Checker for claim-evidence gap issues.

    Claim-evidence gaps occur when papers make claims (especially in
    abstracts, introductions, or conclusions) that are not adequately
    supported by the experimental results, or when results are
    misrepresented or overstated.

    Common types of claim-evidence gaps:
    - Claims in abstract/introduction lacking supporting evidence in results
    - Quantitative claims without corresponding experimental data
    - Generalizations beyond what experiments demonstrate
    ...
    Example:
        >>> checker = ClaimEvidenceGapChecker(claude_client)
    ...
    def __init__(self, claude_client):
        """Initialize claim-evidence gap checker.

        Args:
            claude_client: ClaudeClient instance for API calls
        """
        super().__init__(claude_client, category="claim_evidence_gap")

    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for claim-evidence gap issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for claim-evidence gaps found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check claim-evidence gap: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse claim-evidence gap check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Claim-evidence gap detected"),
