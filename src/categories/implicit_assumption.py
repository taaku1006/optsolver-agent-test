"""Implicit assumption category checker.

Detects unstated or poorly justified assumptions that may limit the
validity or generalizability of a method or its evaluation.
"""
...
class ImplicitAssumptionChecker(BaseChecker):
    """Checker for implicit assumption issues.

    Implicit assumptions are unstated or poorly justified premises that
    underlie a method's design or evaluation. When these assumptions are
    violated in practice, the method may fail or perform poorly in ways
    not addressed by the paper.

    Common types of implicit assumptions:
    - Unstated data distribution assumptions
    - Hidden assumptions about problem structure
    - Implicit computational resource constraints
    - Unstated temporal or spatial assumptions
    - Missing discussion of boundary conditions
    - Assumptions about user behavior or interaction
    - Implicit assumptions about scale or size
    - Unstated requirements for method applicability
    - Hidden assumptions in problem formulation
    - Missing specification of failure modes

    Example:
        >>> checker = ImplicitAssumptionChecker(claude_client)
    ...
    def __init__(self, claude_client):
        """Initialize implicit assumption checker.

        Args:
            claude_client: ClaudeClient instance for API calls
        """
        super().__init__(claude_client, category="implicit_assumption")

    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for implicit assumption issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for implicit assumption issues found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check implicit assumption: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse implicit assumption check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Implicit assumption detected"),
