"""Data leakage category checker.

Detects potential data leakage between train/validation/test sets, which
can invalidate experimental results by allowing test data to influence
...
class DataLeakageChecker(BaseChecker):
    """Checker for data leakage issues.

    Data leakage occurs when information from the test set influences
    model training or selection, invalidating experimental results.
    This is a CRITICAL issue that undermines the validity of any
    performance claims.
    ...
    - Missing clear description of train/validation/test splits

    Example:
        >>> checker = DataLeakageChecker(claude_client)
    ...
    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for data leakage issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for data leakage issues found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check data leakage: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse data leakage check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Data leakage detected"),
