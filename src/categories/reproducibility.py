"""Reproducibility category checker.

Detects insufficient information or practices that prevent independent
researchers from reproducing the reported results.
"""
...
class ReproducibilityChecker(BaseChecker):
    """Checker for reproducibility issues.

    Reproducibility is a cornerstone of scientific research. Papers should
    provide sufficient detail, code, and data for independent researchers
    to reproduce the reported results. Missing or insufficient information
    prevents verification and validation of the research.

    Common types of reproducibility issues:
    - Missing or incomplete implementation details
    - Hyperparameters not fully specified
    - No code or pseudocode provided
    - Data unavailable or inadequately described
    - Missing random seed or initialization details
    - Insufficient description of experimental setup
    - Hardware/software requirements not specified
    - Training procedures incompletely described
    - Missing details about data preprocessing
    - No information about computational requirements
    - Proprietary or closed-source components

    Example:
        >>> checker = ReproducibilityChecker(claude_client)
    ...
    def __init__(self, claude_client):
        ...
        """
        super().__init__(claude_client, category="reproducibility")

    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for reproducibility issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for reproducibility issues found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check reproducibility: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse reproducibility check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Reproducibility issue detected"),
