"""Unfair comparison category checker.

Detects unfair or misleading comparisons with baseline methods, which
can mislead readers about the true contribution and performance of the
proposed method.
"""
...
class UnfairComparisonChecker(BaseChecker):
    """Checker for unfair comparison issues.

    Unfair comparisons occur when the proposed method is evaluated under
    different or more favorable conditions than baseline methods, making
    it impossible to fairly assess the true contribution.

    Common types of unfair comparison:
    ...
    - Proposed method using more data, compute, or iterations
    ...
    Example:
        >>> checker = UnfairComparisonChecker(claude_client)
    ...
    def __init__(self, claude_client):
        """Initialize unfair comparison checker.

        Args:
            claude_client: ClaudeClient instance for API calls
        """
        super().__init__(claude_client, category="unfair_comparison")

    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for unfair comparison issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for unfair comparison issues found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check unfair comparison: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse unfair comparison check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Unfair comparison detected"),
