"""Cherry picking category checker.

Detects selective reporting of results that presents an incomplete or
misleading picture of a method's performance.
"""
...
class CherryPickingChecker(BaseChecker):
    """Checker for cherry picking issues.

    Cherry picking occurs when results are selectively reported to present
    the proposed method in the best possible light, while hiding or
    downplaying negative or inconvenient results. This misleads readers
    about the true performance and limitations of the method.

    Common types of cherry picking:
    - Reporting only favorable metrics while omitting others
    - Showing results only on datasets where method performs well
    - Highlighting best-case scenarios without worst-case analysis
    - Selective reporting of experimental conditions
    - Omitting failed experiments or negative results
    - Cherry-picking specific baselines for comparison
    - Reporting aggregate results that hide individual failures
    - Selective presentation of qualitative examples

    Example:
        >>> checker = CherryPickingChecker(claude_client)
    ...
    def __init__(self, claude_client):
        """Initialize cherry picking checker.

        Args:
            claude_client: ClaudeClient instance for API calls
        """
        super().__init__(claude_client, category="cherry_picking")

    def check(self, paper_data: PaperData) -> list[MethodologicalIssue]:
        """Check paper data for cherry picking issues.

        Args:
            paper_data: Structured paper data from Stage 1 extraction

        Returns:
            List of MethodologicalIssue objects for cherry picking issues found.
        ...
        try:
            ...
            raise ClaudeClientError(
                f"Failed to check cherry picking: {str(e)}"
            ) from e
        except Exception as e:
            raise ValueError(
                f"Failed to parse cherry picking check results: {str(e)}"
            ) from e
    ...
    def _dict_to_issue(self, issue_dict: dict) -> Optional[MethodologicalIssue]:
        ...
        try:
            ...
            return MethodologicalIssue(
                ...
                title=issue_dict.get("title", "Cherry picking detected"),
